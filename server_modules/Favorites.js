...
router.get('/showFavoritePoint', function (req,res) {
...
router.get('/lastSavedPoints', function (req,res) {
    ...
    var userName= decoded.payload.userName;
    var ans=[];
    DButilsAzure.execQuery("SELECT p.pointName, p.picture, p.description FROM (SELECT TOP 2 * FROM UserFavorites Where userName='" + userName + "' order by orderNumber DESC) a JOIN PointsOfInterest p ON a.pointOfInterest=p.pointName").then(function(result){
        res.send(result);
    }).catch(function (err) { res.status(400).send(err); });
});
